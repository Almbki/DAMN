...
export default function GoalScreen() {
  ...
  function startDecompose() {
    setPreview(decomposePending());
  }

  async function confirm() {
    if (!preview) return;
    await confirmDecompose(preview);
    setPreview(null);
  }
  ...
  return (
    <View style={styles.page}>
      ...
      <View style={styles.fabBar}>
        ...
        <ExtendedFab
          icon="target"
          label={pendingTasks.length > 0 ? `拆解 · ${pendingTasks.length}` : '拆解'}
      ...
      {preview ? (
        <DecomposePreviewDialog
          mode={mode}
          tasks={preview}
          onRegenerate={(feedback) => setPreview(decomposePending(feedback))}
          onConfirm={confirm}
