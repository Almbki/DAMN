...
def init_state(profile: UserProfileData) -> UserStateData:
    ...
    for dim in MBTI_DIMENSIONS:
        ...
        # ``weight`` is the pull toward the FIRST letter of the pair, so anchor
        # the two templates on the LETTERS rather than on the resolved type.
        # Anchoring on ``resolved`` (the majority letter) inverted the blend for
        # weights < 0.5: `ie=0.2` (20% I / 80% E) produced an I-weighted state.
        # When ``resolved`` already carries the majority letter, ``first_type``
        # and ``second_type`` are exactly ``resolved`` and its flip, so the
        # previous behaviour for weights >= 0.5 is unchanged.
        idx = MBTI_DIMENSIONS.index(dim)
        first_letter, second_letter = DIMENSION_LETTERS[dim]
        first_type = resolved[:idx] + first_letter + resolved[idx + 1 :]
        second_type = resolved[:idx] + second_letter + resolved[idx + 1 :]
        if first_type not in MBTI_TEMPLATES or second_type not in MBTI_TEMPLATES:
            continue
        first_tmpl = MBTI_TEMPLATES[first_type]
        second_tmpl = MBTI_TEMPLATES[second_type]
        for f in _NUMERIC_FIELDS:
            values[f] = weight * first_tmpl[f] + (1.0 - weight) * second_tmpl[f]
