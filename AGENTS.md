...
`docs/fin/` and `docs/design/` are the **final consensus** — the joint source of
truth for this project. `docs/fin/` wins on any conflict. Priority order:
...
| **1 (co-final)** | `docs/design/` | Co-final design authority; on conflict with `docs/fin/`, `docs/fin/` wins |
| **2** | `ideas and structures/` | Early design notes/prompts (e.g. `ideas and structures/Prompts/画像.md`); may be absorbed or overridden by `docs/fin/` / `docs/design/` |
...
- On conflict: **change the code or the older doc, not `docs/fin/`** (and not
  `docs/design/`). If a `docs/fin/` file genuinely must change, append an entry
  to the 「冲突登记」 table in `docs/fin/核心闭环.md` so the change is traceable.
